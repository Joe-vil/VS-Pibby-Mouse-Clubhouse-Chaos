...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'club-house':
				...
				var fire:BGSprite = new BGSprite('mose/mickeybackground', -1353.65, -215.55, 1, 1, ['clouds'], true);
					fire.antialiasing = true;
				var ground:BGSprite = new BGSprite('mose/mouse house', -1682.4, -387.15, 1, 1);
					ground.antialiasing = true;
				var sky:BGSprite = new BGSprite('mose/sky', -1795.75, -1403.6, 0.8, 0.9);
					sky.antialiasing = true;
				var hue:BGSprite = new BGSprite('mose/hue-black', -1755, -1172, 0.8, 0.9);
					hue.antialiasing = true;
					piggy = new BGSprite('mose/pibby', 1382.05, 732.9, 1, 1, ['pibby'], true);
					piggy.antialiasing = true;

					shitassjumpScare = new FlxSprite().loadGraphic(Paths.image('mose/JUMP'));
					shitassjumpScare.setGraphicSize(Std.int(FlxG.width * jumpscareSizeInterval), Std.int(FlxG.height * jumpscareSizeInterval));
					shitassjumpScare.updateHitbox();
					shitassjumpScare.screenCenter();
	
					shitassjumpScare.setGraphicSize(Std.int(FlxG.width * jumpscareSizeInterval), Std.int(FlxG.height * jumpscareSizeInterval));
					shitassjumpScare.updateHitbox();
					shitassjumpScare.screenCenter();
	
					shitassjumpScare.visible = false;
					shitassjumpScare.cameras = [camHUD];

					add(sky);
					add(fire);
					add(ground);
					add(hue);
					add(shitassjumpScare);
		}
	...
	var shitassjumpScare:FlxSprite;
